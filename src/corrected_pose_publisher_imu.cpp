...
private:
  ...
  tf2::Quaternion computeRotationBetweenVectors(const tf2::Vector3& vec1, const tf2::Vector3& vec2);
...
: Node("corrected_pose_publisher_imu"),
  sum_accel_fcu_(0.0, 0.0, 0.0),
  sum_accel_lidar_(0.0, 0.0, 0.0),
  sample_count_(0),
  calibration_done_(false),
  imu_received_fcu_(false),
  imu_received_lidar_(false)
{
  ...
  imu_sub_fcu_ = this->create_subscription<sensor_msgs::msg::Imu>(
    ...
    std::bind(&CorrectedPosePublisherIMU::imuCallbackFCU, this, std::placeholders::_1));

  imu_sub_lidar_ = this->create_subscription<sensor_msgs::msg::Imu>(
    "/iris0/lidar/imu", qos_settings,
    std::bind(&CorrectedPosePublisherIMU::imuCallbackLiDAR, this, std::placeholders::_1));
  ...
  RCLCPP_INFO(this->get_logger(), "Corrected Pose Publisher IMU node has been started. Calibrating for 5 seconds...");
}

void CorrectedPosePublisherIMU::imuCallbackFCU(const sensor_msgs::msg::Imu::SharedPtr imu_msg)
{
  if (!calibration_done_) {
    tf2::Vector3 accel_fcu(
...
void CorrectedPosePublisherIMU::imuCallbackLiDAR(const sensor_msgs::msg::Imu::SharedPtr imu_msg)
{
  if (!calibration_done_) {
    tf2::Vector3 accel_lidar(
      imu_msg->linear_acceleration.x,
      imu_msg->linear_acceleration.y,
      imu_msg->linear_acceleration.z);
    ...
    if (elapsed_time.seconds() >= 5.0) {
      if (sample_count_ == 0 || !imu_received_fcu_ || !imu_received_lidar_) {
        RCLCPP_ERROR(this->get_logger(), "No IMU data received during calibration.");
        calibration_done_ = true;
        return;
      }

      tf2::Vector3 avg_accel_fcu = sum_accel_fcu_ / static_cast<double>(sample_count_);
      tf2::Vector3 avg_accel_lidar = sum_accel_lidar_ / static_cast<double>(sample_count_);

      double norm_fcu = avg_accel_fcu.length();
      double norm_lidar = avg_accel_lidar.length();

      if (norm_fcu < 1e-6 || norm_lidar < 1e-6) {
        RCLCPP_ERROR(this->get_logger(), "Average acceleration vector is zero, cannot compute correction.");
        calibration_done_ = true;
        return;
      }
      ...
      RCLCPP_INFO(this->get_logger(), "Avg Accel FCU: [%.6f, %.6f, %.6f]", avg_accel_fcu.x(), avg_accel_fcu.y(), avg_accel_fcu.z());
      RCLCPP_INFO(this->get_logger(), "Avg Accel LiDAR: [%.6f, %.6f, %.6f]", avg_accel_lidar.x(), avg_accel_lidar.y(), avg_accel_lidar.z());

      correction_quaternion_ = computeRotationBetweenVectors(avg_accel_lidar, avg_accel_fcu);
...
tf2::Quaternion CorrectedPosePublisherIMU::computeRotationBetweenVectors(const tf2::Vector3& vec1, const tf2::Vector3& vec2)
{
...
void CorrectedPosePublisherIMU::poseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr pose_msg)
{
  if (!calibration_done_) {
    return;
  }
  ...
  if (std::isnan(q_enu.x()) || std::isnan(q_enu.y()) ||
      std::isnan(q_enu.z()) || std::isnan(q_enu.w())) {
    RCLCPP_ERROR(this->get_logger(), "Corrected orientation contains NaN values. Skipping publish.");
    return;
  }
