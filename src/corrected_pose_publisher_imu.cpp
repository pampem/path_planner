...
private:
  void imu_callback_fcu(sensor_msgs::msg::Imu::SharedPtr imu_msg);
  void imu_callback_li_dar(sensor_msgs::msg::Imu::SharedPtr imu_msg);
  void pose_callback(geometry_msgs::msg::PoseStamped::SharedPtr pose_msg);
  static tf2::Quaternion compute_rotation_between_vectors(
  ...
  sum_accel_lidar_(0.0, 0.0, 0.0)
{
  ...
  pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
    "/glim_ros/pose", 10,
    std::bind(&CorrectedPosePublisherIMU::pose_callback, this, std::placeholders::_1));
  ...
  imu_sub_fcu_ = this->create_subscription<sensor_msgs::msg::Imu>(
    ...
    std::bind(&CorrectedPosePublisherIMU::imu_callback_fcu, this, std::placeholders::_1));

  imu_sub_lidar_ = this->create_subscription<sensor_msgs::msg::Imu>(
    "/iris0/lidar/imu", qos_settings,
    std::bind(&CorrectedPosePublisherIMU::imu_callback_li_dar, this, std::placeholders::_1));

  // Publishers for the corrected pose
  vision_pose_pub_ =
    this->create_publisher<geometry_msgs::msg::PoseStamped>("/drone1/mavros/vision_pose/pose", 10);
  mocap_pose_pub_ =
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "Corrected Pose Publisher IMU node has been "
    "started. Calibrating for 5 seconds...");
}

void CorrectedPosePublisherIMU::imu_callback_fcu(const sensor_msgs::msg::Imu::SharedPtr imu_msg)
{
...
void CorrectedPosePublisherIMU::imu_callback_li_dar(const sensor_msgs::msg::Imu::SharedPtr imu_msg)
{
  if (!calibration_done_) {
    ...
    if (elapsed_time.seconds() >= 5.0) {
      ...
      if (norm_fcu < 1e-6 || norm_lidar < 1e-6) {
        RCLCPP_ERROR(
          this->get_logger(), "Average acceleration vector is zero, cannot compute correction.");
        calibration_done_ = true;
        return;
      }
      ...
      RCLCPP_INFO(
        this->get_logger(), "Avg Accel FCU: [%.6f, %.6f, %.6f]", avg_accel_fcu.x(),
        avg_accel_fcu.y(), avg_accel_fcu.z());
      RCLCPP_INFO(
        this->get_logger(), "Avg Accel LiDAR: [%.6f, %.6f, %.6f]", avg_accel_lidar.x(),
        avg_accel_lidar.y(), avg_accel_lidar.z());

      correction_quaternion_ = compute_rotation_between_vectors(avg_accel_lidar, avg_accel_fcu);
...
tf2::Quaternion CorrectedPosePublisherIMU::compute_rotation_between_vectors(
  const tf2::Vector3 & vec1, const tf2::Vector3 & vec2)
{
...
void CorrectedPosePublisherIMU::pose_callback(
  const geometry_msgs::msg::PoseStamped::SharedPtr pose_msg)
{
  ...
  if (
    std::isnan(q_enu.x()) || std::isnan(q_enu.y()) || std::isnan(q_enu.z()) ||
    std::isnan(q_enu.w())) {
    RCLCPP_ERROR(
      this->get_logger(), "Corrected orientation contains NaN values. Skipping publish.");
    return;
  }
